...
| `agents/prompt_constants.py` | All 5 agents, `docs/INDICATOR_CONFIDENCE_MATRIX.md` |
...
### 8. Agent Data Flow Integrity

Flag if:
- Downstream agents receive truncated data instead of `_raw_*` fields
- Judge receives `summary` instead of `_raw_summary`
- Entry Timing / Risk Manager receives `rationale` instead of `_raw_rationale`
- `AIQualityAuditor` checks truncated fields instead of `_raw_*` versions

### 9. R/R Ratio Guarantees

Flag if:
- `calculate_mechanical_sltp()` no longer guarantees R/R >= 2.0:1
...
- Missing error handling on external API calls (Binance, DeepSeek, Coinalyze, Telegram)
...
| `agents/multi_agent_analyzer.py` | AI decision pipeline | Schema validation, data truncation, prompt injection |
| `agents/ai_quality_auditor.py` | Quality scoring | Regex precision, cross-TF attribution, false positives |
