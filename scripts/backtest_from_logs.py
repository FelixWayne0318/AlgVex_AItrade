#!/usr/bin/env python3
"""
Backtest v37.1 using real AI signals extracted from journalctl logs.
...
# v37.1 Plan II — wider SL to reduce circuit breaker cascade
PLAN_II = {
    "label": "Plan II (v37.1)",
...
BASELINE = {
    "label": "Baseline (old, conf>=MEDIUM)",
...
# NOTE: This backtest uses 30M bars for ATR. When 4H ATR is unavailable,
# production falls back to 30M ATR with these smaller multipliers, resulting
# in tighter SL than Plan I/II. This plan exists for parameter comparison.
PLAN_V39 = {
    "label": "Plan v39.0 (4H ATR scale)",
...
PLAN_V40A = {
    "label": "Plan V40a (v39 SL, R/R=1.5/1.3)",
    "sl_atr_multiplier": {"HIGH": 0.8, "MEDIUM": 1.0, "LOW": 1.0},
...
# v40b: Keep v39.0 SL, even lower R/R target (TP ~40% closer, scalping-like)
PLAN_V40B = {
    "label": "Plan V40b (v39 SL, R/R=1.3/1.2)",
    "sl_atr_multiplier": {"HIGH": 0.8, "MEDIUM": 1.0, "LOW": 1.0},
...
PLAN_V40C = {
    "label": "Plan V40c (tight SL=0.6/0.8, R/R=1.5/1.3)",
...
PYRAMIDING_MIN_CONFIDENCE = "HIGH"  # Only HIGH confidence can pyramid
...
def run_backtest(signals: List[Dict], use_gates: bool = True):
    ...
    # Plan I with production gates
    logger.info(f"Backtesting: Plan I (v37.0) {'with' if use_gates else 'without'} production gates...")
    sim_gates = ProductionSimulator(PLAN_I, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    res_gates = sim_gates.run(valid_signals)
    results["plan_i_gates"] = res_gates

    # Plan I without gates (v1 mode for comparison)
    logger.info("Backtesting: Plan I (v37.0) without gates (v1 mode)...")
    sim_nogates = ProductionSimulator(PLAN_I, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    res_nogates = sim_nogates.run(valid_signals)
    results["plan_i_nogates"] = res_nogates

    # Plan II with production gates
    logger.info(f"Backtesting: Plan II (v37.1) {'with' if use_gates else 'without'} production gates...")
    sim_p2_gates = ProductionSimulator(PLAN_II, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    res_p2_gates = sim_p2_gates.run(valid_signals)
    results["plan_ii_gates"] = res_p2_gates

    # Plan II without gates
    logger.info("Backtesting: Plan II (v37.1) without gates (v1 mode)...")
    sim_p2_nogates = ProductionSimulator(PLAN_II, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    res_p2_nogates = sim_p2_nogates.run(valid_signals)
    results["plan_ii_nogates"] = res_p2_nogates

    # Baseline without gates
    logger.info("Backtesting: Baseline (old, conf>=MEDIUM) without gates...")
    sim_baseline = ProductionSimulator(BASELINE, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    res_baseline = sim_baseline.run(valid_signals)
    results["baseline_nogates"] = res_baseline

    # Baseline with gates
    logger.info("Backtesting: Baseline (old, conf>=MEDIUM) with gates...")
    sim_baseline_g = ProductionSimulator(BASELINE, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=True)
    res_baseline_g = sim_baseline_g.run(valid_signals)
    results["baseline_gates"] = res_baseline_g

    # v39.0 with gates
    logger.info(f"Backtesting: v39.0 (4H ATR scale) {'with' if use_gates else 'without'} production gates...")
    sim_v39_gates = ProductionSimulator(PLAN_V39, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    res_v39_gates = sim_v39_gates.run(valid_signals)
    results["v39_gates"] = res_v39_gates

    # v39.0 without gates
    logger.info("Backtesting: v39.0 (4H ATR scale) without gates (v1 mode)...")
    sim_v39_nogates = ProductionSimulator(PLAN_V39, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    res_v39_nogates = sim_v39_nogates.run(valid_signals)
    results["v39_nogates"] = res_v39_nogates

    # v40a: v39 SL + lower R/R (1.5/1.3)
    logger.info(f"Backtesting: V40a (R/R=1.5/1.3) {'with' if use_gates else 'without'} production gates...")
    sim_v40a_gates = ProductionSimulator(PLAN_V40A, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    results["v40a_gates"] = sim_v40a_gates.run(valid_signals)

    logger.info("Backtesting: V40a (R/R=1.5/1.3) without gates...")
    sim_v40a_nogates = ProductionSimulator(PLAN_V40A, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    results["v40a_nogates"] = sim_v40a_nogates.run(valid_signals)

    # v40b: v39 SL + even lower R/R (1.3/1.2)
    logger.info(f"Backtesting: V40b (R/R=1.3/1.2) {'with' if use_gates else 'without'} production gates...")
    sim_v40b_gates = ProductionSimulator(PLAN_V40B, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    results["v40b_gates"] = sim_v40b_gates.run(valid_signals)

    logger.info("Backtesting: V40b (R/R=1.3/1.2) without gates...")
    sim_v40b_nogates = ProductionSimulator(PLAN_V40B, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    results["v40b_nogates"] = sim_v40b_nogates.run(valid_signals)

    # v40c: Tighter SL + lower R/R
    logger.info(f"Backtesting: V40c (tight SL, R/R=1.5/1.3) {'with' if use_gates else 'without'} production gates...")
    sim_v40c_gates = ProductionSimulator(PLAN_V40C, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=use_gates)
    results["v40c_gates"] = sim_v40c_gates.run(valid_signals)

    logger.info("Backtesting: V40c (tight SL, R/R=1.5/1.3) without gates...")
    sim_v40c_nogates = ProductionSimulator(PLAN_V40C, PRODUCTION_GATES, bars_1m, bars_30m, use_gates=False)
    results["v40c_nogates"] = sim_v40c_nogates.run(valid_signals)
    ...
    print(f"{'=' * 150}")
    print(f"  {'方案':<50} {'信号':>4} {'跳过':>5} {'仓位':>4} {'TP':>3} {'SL':>3} {'TB':>3} {'Trail':>5} "
          f"{'加仓':>4} {'胜率':>6} {'PnL':>8} {'回撤':>7} {'净值':>8}")
    print(f"  {'-' * 140}")

    for key, label in [
        ("v39_gates", f"★ v39.0 (4H ATR) + Gate ({'ON' if use_gates else 'OFF'})"),
        ("v39_nogates", "  v39.0 (4H ATR) 无 Gate (v1 模式)"),
        ("v40a_gates", f"★ V40a (R/R=1.5/1.3) + Gate ({'ON' if use_gates else 'OFF'})"),
        ("v40a_nogates", "  V40a (R/R=1.5/1.3) 无 Gate"),
        ("v40b_gates", f"★ V40b (R/R=1.3/1.2) + Gate ({'ON' if use_gates else 'OFF'})"),
        ("v40b_nogates", "  V40b (R/R=1.3/1.2) 无 Gate"),
        ("v40c_gates", f"★ V40c (紧SL, R/R=1.5/1.3) + Gate ({'ON' if use_gates else 'OFF'})"),
        ("v40c_nogates", "  V40c (紧SL, R/R=1.5/1.3) 无 Gate"),
        ("plan_ii_gates", f"  Plan II + 生产 Gate ({'ON' if use_gates else 'OFF'})"),
        ("plan_ii_nogates", "  Plan II 无 Gate (v1 模式)"),
        ("plan_i_gates", f"  Plan I + 生产 Gate ({'ON' if use_gates else 'OFF'})"),
        ("plan_i_nogates", "  Plan I 无 Gate (v1 模式)"),
        ("baseline_gates", "  Baseline + 生产 Gate"),
        ("baseline_nogates", "  Baseline 无 Gate (v1 模式)"),
    ]:
        r = results[key]
        s = r["summary"]
        ...
        wr = s.get("win_rate", 0)
        print(
            f"  {label:<50} {s['total_signals_processed']:>4} "
    ...
    for key, label in [("v39_gates", "v39.0"), ("v40a_gates", "V40a"), ("v40b_gates", "V40b"), ("v40c_gates", "V40c"), ("plan_ii_gates", "Plan II"), ("plan_i_gates", "Plan I"), ("baseline_gates", "Baseline")]:
        r = results[key]
    ...
    # ── Per-trade detail for Plan II with gates ──
    plan_detail = results["plan_ii_gates"]
    active_trades = [t for t in plan_detail["trades"] if t["outcome"] != "FILTERED"]

    if active_trades:
        print(f"\n{'=' * 150}")
        print(f"  Plan II + 生产 Gate — 每笔详情 ({len(active_trades)} 笔)")
        print(f"{'=' * 150}")
    ...
    if closed_trades:
        ...
        if blocked_trades or executed_trades:
            print(f"\n{'=' * 120}")
            print(f"  Plan II + Gate: 原被拦截 vs 原已执行")
            print(f"{'=' * 120}")
    ...
    # ── Save results ──
    output_path = Path(__file__).parent.parent / "data" / "backtest_v37_from_logs.json"
    output = {
        ...
        "params": {"plan_i": PLAN_I, "plan_ii": PLAN_II, "baseline": BASELINE, "v39": PLAN_V39, "v40a": PLAN_V40A, "v40b": PLAN_V40B, "v40c": PLAN_V40C},
        "results": {
            key: {
                "summary": r["summary"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AlgVex v37.1 回测 v3.0 (多层加仓+Trailing)")
