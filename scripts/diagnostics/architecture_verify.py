...
class TradingAgentsArchitectureVerifier(DiagnosticStep):
    ...
    def _verify_v18_batch_features(self) -> None:
        ...
        # Check 3: _format_direction_report exists for data partitioning
        if "def _format_direction_report(" in ma_source:
            checks_passed += 1
        else:
            errors.append("_format_direction_report() not found in MultiAgentAnalyzer")

        # Check 4: _audit_direction_compliance method exists
        if "def _audit_direction_compliance(" in ma_source:
            checks_passed += 1
        else:
            errors.append("_audit_direction_compliance() not found in MultiAgentAnalyzer")
    ...
    def _verify_v191_features(self) -> None:
        ...
        # Check 5: SIGNAL_CONFIDENCE_MATRIX has Ext Ratio rows
        if "Ext Ratio >3" in ma_source and "Ext Ratio >5" in ma_source:
            checks_passed += 1
        else:
            errors.append("SIGNAL_CONFIDENCE_MATRIX missing Ext Ratio rows")
    ...
    def _verify_v192_features(self) -> None:
        ...
        # Check 5: SIGNAL_CONFIDENCE_MATRIX has absorption and OI×CVD rows
        if "CVD absorption" in ma_source and "OI×CVD positioning" in ma_source:
            checks_passed += 1
        else:
            errors.append("SIGNAL_CONFIDENCE_MATRIX missing CVD absorption / OI×CVD rows")
    ...
    def _verify_v200_features(self) -> None:
        ...
        # Check 7: SIGNAL_CONFIDENCE_MATRIX has Vol Regime + OBV rows
        if ("Vol LOW" in ma_source or "Vol HIGH" in ma_source) and "OBV divergence" in ma_source:
            checks_passed += 1
        else:
            errors.append("SIGNAL_CONFIDENCE_MATRIX missing Vol Regime / OBV divergence rows")
    ...
    def _verify_v270_schema_infrastructure(self) -> None:
        ...
        # Check 5: replay_ab_compare.py exists
        try:
            script_path = Path(__file__).resolve().parent.parent / "replay_ab_compare.py"
            if script_path.exists():
                checks_passed += 1
            else:
                errors.append("scripts/replay_ab_compare.py not found")
        except Exception:
            errors.append("Could not check replay_ab_compare.py existence")

        # Check 6: _validate_agent_output exists with schema violation tracking
        try:
            if "_schema_violations" in ma_source and "_validate_agent_output" in ma_source:
                checks_passed += 1
            else:
                errors.append("_validate_agent_output or _schema_violations tracking not found")
        except Exception:
            errors.append("Could not verify schema validation infrastructure")
    ...
    def _verify_v390_v400_features(self) -> None:
        ...
        try:
            ...
            ok = (sl.get('HIGH') == 0.8 and sl.get('MEDIUM') == 1.0
                  and tp.get('HIGH') == 1.5 and tp.get('MEDIUM') == 1.3
                  and floor == 0.5)
            if ok:
                checks_passed += 1
                print(f"     ✅ [A4] v40.0 SL/TP params: SL={sl}, TP={tp}, floor={floor}")
            else:
    ...
    def _verify_ai_decision(self) -> None:
        ...
        # v29+: _last_analysis_context persisted on analyzer
        ma = self.ctx.multi_agent
        if ma:
            last_ctx = getattr(ma, '_last_analysis_context', None)
            if last_ctx is not None:
                print(f"     ✅ _last_analysis_context persisted on MultiAgentAnalyzer")
            else:
                print(f"     ⚠️ _last_analysis_context not found on MultiAgentAnalyzer")
