...
class CodeIntegrityChecker(DiagnosticStep):
    ...
    def run(self) -> bool:
        ...
        # P1.97-P1.99: v24.0 AI Quality Auditor integration
        ...
        _auditor_path = self.ctx.project_root / "agents" / "ai_quality_auditor.py"
        auditor_source = _auditor_path.read_text(encoding='utf-8') if _auditor_path.exists() else ""
        self._check_v340_tag_classification(auditor_source)
        self._check_v340_reason_signal_conflict(auditor_source)
        self._check_v340_confidence_risk_conflict(auditor_source)
    ...
    def _check_v18_data_partitioning(self, ma_source: str) -> None:
        """P1.62: v18 Item 22 — Per-agent data partitioning.

        Verifies Bull/Bear receive direction-only data in trending markets.
        """
        has_format_direction = "def _format_direction_report(" in ma_source
        has_direction_report_var = "direction_report" in ma_source
        # Should use full data in ranging (ADX<25)
        has_ranging_bypass = "adx_1d_value < 25" in ma_source or "adx_1d < 25" in ma_source
        has_alignment_data = "_alignment_data" in ma_source

        ok = has_format_direction and has_direction_report_var and has_ranging_bypass
        self._record("P1.62", "v18 Item 22: Per-agent data partitioning", ok,
                     expected="_format_direction_report + direction_report variable + ADX<25 bypass",
                     actual=f"method={has_format_direction}, var={has_direction_report_var}, "
                            f"ranging={has_ranging_bypass}, alignment={has_alignment_data}")
    ...
    def _check_v191_signal_confidence_matrix(self, ma_source: str) -> None:
        """P1.79: v19.1 SIGNAL_CONFIDENCE_MATRIX contains Ext Ratio >3 and >5 rows."""
        if not ma_source:
            self._record("P1.79", "v19.1 SIGNAL_CONFIDENCE_MATRIX Ext Ratio rows", False,
                         actual="multi_agent_analyzer.py not found")
            return

        has_overextended_row = "Ext Ratio >3" in ma_source and "overextended" in ma_source
        has_extreme_row = "Ext Ratio >5" in ma_source and "extreme" in ma_source
        # Verify correct ADX>40 de-emphasis multiplier for overextended (0.7)
        has_adx40_deemphasis = "0.7" in ma_source

        ok = has_overextended_row and has_extreme_row
        self._record("P1.79", "v19.1 SIGNAL_CONFIDENCE_MATRIX Ext Ratio rows", ok,
                     expected="'Ext Ratio >3 (overextended)' + 'Ext Ratio >5 (extreme)' rows",
                     actual=f"overextended_row={has_overextended_row}, extreme_row={has_extreme_row}, "
                            f"adx40_deemphasis={has_adx40_deemphasis}")
    ...
    def _check_v192_oi_cvd_bridge(self, ma_source: str) -> None:
        """P1.83: v19.2 OI×CVD cross-analysis in derivatives report.

        CoinGlass 4-quadrant framework: OI↑+CVD↑=longs opening, OI↑+CVD↓=shorts opening,
        OI↓+CVD↓=longs closing, OI↓+CVD↑=shorts closing.
        """
        if not ma_source:
            self._record("P1.83", "v19.2 OI×CVD bridge", False,
                         actual="multi_agent_analyzer.py not found")
            return

        # Check for the 4-quadrant signals
        has_longs_opening = "LONGS OPENING" in ma_source
        has_shorts_opening = "SHORTS OPENING" in ma_source
        has_longs_closing = "LONGS CLOSING" in ma_source
        has_shorts_closing = "SHORTS CLOSING" in ma_source
        # Check that _format_derivatives_report accepts cvd_data parameter
        has_cvd_param = "cvd_data" in ma_source
        # Check oi_cvd_map structure
        has_oi_cvd_map = "oi_cvd_map" in ma_source

        ok = (has_longs_opening and has_shorts_opening and has_longs_closing
              and has_shorts_closing and has_cvd_param and has_oi_cvd_map)
        self._record("P1.83", "v19.2 OI×CVD cross-analysis (CoinGlass 4-quadrant framework)", ok,
                     expected="LONGS/SHORTS OPENING/CLOSING + cvd_data param + oi_cvd_map",
                     actual=f"longs_open={has_longs_opening}, shorts_open={has_shorts_opening}, "
                            f"longs_close={has_longs_closing}, shorts_close={has_shorts_closing}, "
                            f"cvd_param={has_cvd_param}, oi_cvd_map={has_oi_cvd_map}")

    # ── P1.84: v19.2 CVD Absorption detection ──

    def _check_v192_absorption_detection(self, ma_source: str) -> None:
        """P1.84: v19.2 CVD Absorption detection (price flat + CVD directional).

        Absorption = large passive orders absorbing aggressive flow without moving price.
        CVD positive + price flat = passive seller absorbing buys.
        CVD negative + price flat = passive buyer absorbing sells.
        """
        if not ma_source:
            self._record("P1.84", "v19.2 CVD Absorption detection", False,
                         actual="multi_agent_analyzer.py not found")
            return

        # Check for ABSORPTION tag in both 30M and 4H
        has_absorption_tag = "CVD-PRICE ABSORPTION" in ma_source
        # Check for 4H absorption
        has_4h_absorption = "4H CVD-PRICE ABSORPTION" in ma_source
        # Check for passive buyer/seller language
        has_passive_seller = "passive seller" in ma_source
        has_passive_buyer = "passive buyer" in ma_source
        # Check INDICATOR_DEFINITIONS mentions absorption
        has_indicator_def = "Absorption" in ma_source and "INDICATOR_DEFINITIONS" in ma_source
        # Check SIGNAL_CONFIDENCE_MATRIX has absorption row
        has_matrix_row = "CVD absorption" in ma_source

        ok = (has_absorption_tag and has_4h_absorption and has_passive_seller
              and has_passive_buyer and has_matrix_row)
        self._record("P1.84", "v19.2 CVD Absorption detection (passive liquidity)", ok,
                     expected="ABSORPTION tags (30M+4H) + passive seller/buyer + SIGNAL_CONFIDENCE_MATRIX row",
                     actual=f"absorption_tag={has_absorption_tag}, 4h_absorption={has_4h_absorption}, "
                            f"passive_seller={has_passive_seller}, passive_buyer={has_passive_buyer}, "
                            f"matrix_row={has_matrix_row}")
    ...
    def _check_v200_volatility_regime_prompts(self, ma_source: str) -> None:
        """P1.89: v20.0 Volatility Regime + OBV in AI prompts and SIGNAL_CONFIDENCE_MATRIX."""
        if not ma_source:
            self._record("P1.89", "v20.0 Volatility Regime + OBV in AI prompts", False,
                         actual="multi_agent_analyzer.py not found")
            return

        # Volatility Regime in INDICATOR_DEFINITIONS
        has_vol_indicator_def = "ATR VOLATILITY REGIME" in ma_source
        # Volatility Regime in technical report
        has_vol_report = "Volatility Regime:" in ma_source and "vol_regime" in ma_source
        # OBV in INDICATOR_DEFINITIONS
        has_obv_indicator_def = "OBV" in ma_source and "DIVERGENCE" in ma_source
        # SIGNAL_CONFIDENCE_MATRIX rows for v20.0
        has_vol_matrix = "Vol LOW" in ma_source or "Vol HIGH" in ma_source or "Vol EXTREME" in ma_source
        has_obv_matrix = "OBV divergence" in ma_source or "OBV+CVD" in ma_source
        # Bull/Bear prompt references
        has_bull_bear_vol = "Volatility Regime" in ma_source

        ok = (has_vol_indicator_def and has_vol_report and has_obv_indicator_def
              and has_vol_matrix and has_obv_matrix and has_bull_bear_vol)
        self._record("P1.89", "v20.0 Volatility Regime + OBV in prompts + SIGNAL_CONFIDENCE_MATRIX", ok,
                     expected="INDICATOR_DEFINITIONS (Vol+OBV) + report + MATRIX rows + Bull/Bear prompts",
                     actual=f"vol_def={has_vol_indicator_def}, vol_report={has_vol_report}, "
                            f"obv_def={has_obv_indicator_def}, vol_matrix={has_vol_matrix}, "
                            f"obv_matrix={has_obv_matrix}, bull_bear={has_bull_bear_vol}")
    ...
    def _check_v240_quality_auditor_import(self, ma_source: str) -> None:
        """P1.97: v24.0 AIQualityAuditor is imported in multi_agent_analyzer."""
        ok = "from agents.ai_quality_auditor import AIQualityAuditor" in ma_source
        self._record("P1.97", "v24.0 AIQualityAuditor imported in multi_agent_analyzer", ok,
                     expected="from agents.ai_quality_auditor import AIQualityAuditor",
                     actual="found" if ok else "not found")

    def _check_v240_quality_audit_call(self, ma_source: str) -> None:
        """P1.98: v24.0 quality audit runs after analysis (try-except protected)."""
        has_audit_call = "_quality_auditor.audit(" in ma_source
        has_try_protect = "AI Quality Audit skipped" in ma_source
        ok = has_audit_call and has_try_protect
        self._record("P1.98", "v24.0 quality audit call in analyze() with try-except", ok,
                     expected="_quality_auditor.audit() + try-except protection",
                     actual=f"audit_call={has_audit_call}, try_protect={has_try_protect}")
    ...
    def _check_v29_confidence_chain(self, ma_source: str) -> None:
        """P1.111: v29+ Confidence chain serialized in final_decision for diagnostic access.
        analyze() must attach _confidence_chain to final_decision and persist _last_analysis_context."""
        has_chain_key = "'_confidence_chain'" in ma_source or '"_confidence_chain"' in ma_source
        has_ctx_persist = "_last_analysis_context" in ma_source
        has_chain_add = "confidence_chain.add(" in ma_source
        ok = has_chain_key and has_ctx_persist and has_chain_add
        self._record("P1.111", "v29+ _confidence_chain in final_decision + _last_analysis_context persisted", ok,
                     expected="_confidence_chain serialized + ctx persisted + chain.add() calls",
                     actual=f"chain_key={has_chain_key}, ctx_persist={has_ctx_persist}, chain_add={has_chain_add}")

    def _check_v29_conditions_v2(self, ma_source: str) -> None:
        """P1.112: v29+ conditions_v2 (_memory_conditions_snapshot) attached to final_decision.
        analyze() must attach MemoryConditions snapshot for downstream event_handlers."""
        has_snapshot_key = "_memory_conditions_snapshot" in ma_source
        has_from_feature_dict = "MemoryConditions.from_feature_dict" in ma_source
        has_quality_key = "_ai_quality_score" in ma_source
        ok = has_snapshot_key and has_from_feature_dict and has_quality_key
        self._record("P1.112", "v29+ _memory_conditions_snapshot + _ai_quality_score in final_decision", ok,
                     expected="MemoryConditions.from_feature_dict() + snapshot + quality_score keys",
                     actual=f"snapshot={has_snapshot_key}, from_feature_dict={has_from_feature_dict}, quality={has_quality_key}")

    # ── P1.113: v32.1 Risk Manager conditional skip ──

    def _check_v321_risk_manager_skip(self, ma_source: str) -> None:
        """P1.113: v32.1 Risk Manager skipped for non-actionable signals.
        When Judge outputs HOLD/CLOSE/REDUCE, Risk Manager API call is skipped
        and safe passthrough defaults are used instead."""
        has_condition = "_final_signal in ('LONG', 'SHORT')" in ma_source
        has_skip_log = "Risk evaluation skipped" in ma_source
        has_passthrough = '"risk_appetite": "NORMAL"' in ma_source and '"position_size_pct": 0' in ma_source
        ok = has_condition and has_skip_log and has_passthrough
        self._record("P1.113", "v32.1 Risk Manager conditional skip (HOLD/CLOSE/REDUCE → passthrough)", ok,
                     expected="Conditional _run_structured_risk + skip log + safe defaults",
                     actual=f"condition={has_condition}, skip_log={has_skip_log}, passthrough={has_passthrough}")

    # ── P1.115-P1.117: v34.0 Auditor logic-level coherence checks ──

    def _check_v340_tag_classification(self, auditor_source: str) -> None:
        """P1.115: v34.0 BULLISH/BEARISH_EVIDENCE_TAGS imported for reason-signal alignment.
        Auditor must import tag classification constants from prompt_constants."""
        has_bullish_import = "BULLISH_EVIDENCE_TAGS" in auditor_source
        has_bearish_import = "BEARISH_EVIDENCE_TAGS" in auditor_source
        ok = has_bullish_import and has_bearish_import
        self._record("P1.115", "v34.0 BULLISH/BEARISH_EVIDENCE_TAGS in auditor", ok,
                     expected="Both tag sets imported for reason-signal alignment",
                     actual=f"bullish={has_bullish_import}, bearish={has_bearish_import}")

    def _check_v340_reason_signal_conflict(self, auditor_source: str) -> None:
        """P1.116: v34.0 reason_signal_conflict field and check method exist."""
        has_field = "reason_signal_conflict" in auditor_source
        has_method = "_check_reason_signal_alignment" in auditor_source
        ok = has_field and has_method
        self._record("P1.116", "v34.0 reason_signal_conflict field + alignment check", ok,
                     expected="reason_signal_conflict field + _check_reason_signal_alignment method",
                     actual=f"field={has_field}, method={has_method}")

    def _check_v340_confidence_risk_conflict(self, auditor_source: str) -> None:
        """P1.117: v34.0 confidence_risk_conflict field and check method exist."""
        has_field = "confidence_risk_conflict" in auditor_source
        has_method = "_check_confidence_risk_coherence" in auditor_source
        ok = has_field and has_method
        self._record("P1.117", "v34.0 confidence_risk_conflict field + coherence check", ok,
                     expected="confidence_risk_conflict field + _check_confidence_risk_coherence method",
                     actual=f"field={has_field}, method={has_method}")
    ...
    def _check_v400_tp_parameter_ssot(self) -> None:
        ...
        try:
            ...
            ok_logic = (tp_logic.get('HIGH') == 1.5 and tp_logic.get('MEDIUM') == 1.3
                        and sl_logic.get('HIGH') == 0.8 and sl_logic.get('MEDIUM') == 1.0)
            ok = ok_logic and sl_hi_match and tp_hi_match
            self._record("P1.121", "v40.0 SL/TP SSoT: trading_logic + backtest_math consistent", ok,
                         expected="TP: HIGH=1.5/MED=1.3, SL: HIGH=0.8/MED=1.0 across SSoT files",
