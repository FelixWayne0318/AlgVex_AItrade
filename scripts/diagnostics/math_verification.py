...
class MathVerificationChecker(DiagnosticStep):
    ...
    def _check_trailing_stop_math(self, price: float):
        ...
        try:
            ...
            atr = (self.ctx.technical_data or {}).get('atr', price * 0.015) if self.ctx.technical_data else price * 0.015
    ...
    def _check_mechanical_sltp_v40(self, price: float):
        ...
        try:
            ...
            expected_tp = {'HIGH': 1.5, 'MEDIUM': 1.3, 'LOW': 1.3}
            ok_b = tp_targets == expected_tp
            self._record("M17b", "v40.0 TP R/R targets match production", ok_b,
            ...
            # M17g: TP distance = SL distance × R/R target (MEDIUM=1.3)
            if success:
                tp_dist_4h = abs(tp_4h - price)
                expected_tp_dist = sl_dist_4h * 1.3  # MEDIUM R/R target
                ok_g = abs(tp_dist_4h - expected_tp_dist) < 0.01
                self._record("M17g", "TP distance = SL × R/R target (MEDIUM=1.3)", ok_g,
