    ...
    def _check_analyzer_memory(self) -> None:
        ...
        if self.ctx.multi_agent is not None:
            ...
            if hasattr(self.ctx.multi_agent, '_get_past_memories'):
                ...
                try:
                    from agents.analysis_context import MemoryConditions
                    if self.ctx.technical_data:
                        # Build a minimal feature dict from technical_data for diagnostics
                        mc = MemoryConditions()  # defaults
                        current_conditions = mc.to_dict()
                        print(f"     → MemoryConditions: ✅ 成功 (diagnostics default)")
                except Exception as e:
