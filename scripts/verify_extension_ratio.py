...
def test_source_code_integration():
    ...
    # Check 2: agent module integrations (core + auxiliary files)
    _agent_files = [
        "agents/multi_agent_analyzer.py", "agents/prompt_constants.py",
        "agents/report_formatter.py", "agents/memory_manager.py",
    ]
    maa_src = ""
    ...
    checks.append((
        "ATR EXTENSION RATIO" in maa_src,
        "INDICATOR_DEFINITIONS contains Extension Ratio guide"
    ))
    checks.append((
        "Ext Ratio" in maa_src,
        "SIGNAL_CONFIDENCE_MATRIX contains extension entries"
    ))
    checks.append((
    ...
    # Check 3: Bull prompt extension references
    # Find _get_bull_argument method
    bull_start = maa_src.find("def _get_bull_argument")
    bull_end = maa_src.find("def _get_bear_argument")
    bull_src = maa_src[bull_start:bull_end] if bull_start > 0 and bull_end > 0 else ""
    checks.append((
        "Extension Ratio" in bull_src,
        "Bull prompt references Extension Ratio"
    ))

    # Check 4: Bear prompt extension references
    bear_start = maa_src.find("def _get_bear_argument")
    bear_end = maa_src.find("def _get_judge_decision")
    bear_src = maa_src[bear_start:bear_end] if bear_start > 0 and bear_end > 0 else ""
    checks.append((
        "Extension Ratio" in bear_src,
        "Bear prompt references Extension Ratio"
    ))

    # Check 5: Judge prompt extension references
    judge_start = maa_src.find("def _get_judge_decision")
    judge_end = maa_src.find("def _build_key_metrics")
    judge_src = maa_src[judge_start:judge_end] if judge_start > 0 and judge_end > 0 else ""
    checks.append((
        "Extension Ratio" in judge_src or "extension" in judge_src.lower(),
        "Judge prompt references extension"
    ))

    # Check 6: Risk Manager prompt extension references
    risk_start = maa_src.find("def _evaluate_risk")
    risk_end = maa_src.find("def _normalize_signal")
    risk_src = maa_src[risk_start:risk_end] if risk_start > 0 and risk_end > 0 else ""
    checks.append((
        "Extension Ratio" in risk_src,
        "Risk Manager prompt references Extension Ratio"
    ))
