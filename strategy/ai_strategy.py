...
from nautilus_trader.model.enums import PositionSide, PriceType, OrderType
...
class AITradingStrategyConfig(StrategyConfig, frozen=True):
    ...
    deepseek_thinking_enabled: bool = True  # v32.0: Enable DeepSeek V3.2 thinking mode
...
class AITradingStrategy(
    ...
    def __init__(self, config: AITradingStrategyConfig):
        ...
        # ===== v12.0: Per-Agent Reflection Memory =====
        # Pending reflections queue (List, not single value — handles reversal edge case)
        # Each entry: {timestamp, memory_entry} — consumed by next on_timer before AI analysis
        self._pending_reflections: List[Dict[str, Any]] = []
        # Judge decision summary captured at entry for reflection context
        self._entry_judge_summary: str = ""
        self._entry_winning_side: str = ""
        # v23.0: Entry Timing assessment captured at entry for memory/reflection
        self._entry_timing_verdict: str = ""
        self._entry_timing_quality: str = ""
        # v29+: AnalysisContext snapshots captured at entry
        self._entry_memory_conditions: Optional[Dict] = None
        self._entry_ai_quality_score: Optional[int] = None
        ...
        # ===== v23.0: Entry Timing Consecutive REJECT Counter =====
        # Track consecutive Entry Timing REJECTs to detect "always REJECT" dead loops.
        # v42.0: ET Exhaustion mechanism (mirrors v21.0 FR exhaustion pattern):
        #   Tier 1 (>=5): ET REJECT overridden → forced ENTER at LOW confidence (30% position)
        #   Tier 2 (>=8): ET skipped entirely, Judge confidence preserved
        #   Reset: successful entry → 0, one-shot trigger → count-3
        self._et_consecutive_rejects: int = 0
        self._ET_EXHAUSTION_TIER1: int = 5   # Override REJECT → ENTER at LOW
        self._ET_EXHAUSTION_TIER2: int = 8   # Skip ET entirely
        ...
        self.multi_agent = MultiAgentAnalyzer(
            ...
            enable_thinking=config.deepseek_thinking_enabled,  # v32.0: thinking mode
        )
        thinking_status = "enabled" if config.deepseek_thinking_enabled else "disabled"
        self.log.info(f"✅ Multi-Agent analyzer initialized (debate_rounds={config.debate_rounds}, thinking={thinking_status})")
    ...
    def on_timer(self, event):
        ...
        try:
            ...
            try:
                ...
                # Important: Add price to technical_data (needed by multi_agent_analyzer._format_technical_report)
                ...
                if not _skip_ai_analysis:
                    ...
                    # v42.1: ET Exhaustion Tier 1 — pass flag so analyze() can
                    # override REJECT BEFORE Risk Manager (not after).
                    _exhaustion_tier1 = (
                        not _skip_et
                        and self._et_consecutive_rejects >= self._ET_EXHAUSTION_TIER1
                    )
                    ...
                    signal_data = self.multi_agent.analyze(
                        ...
                        # ========== MTF v2.1 Additional Parameters ==========
                        ...
                        # ========== v3.7 Additional Parameters ==========
                        orderbook_report=orderbook_data,
                        # ========== v4.6 Additional Parameters ==========
                        ...
                        # v6.6: Pass data quality warnings so AI agents know which sources are degraded
                        ...
                        # v42.0: ET Exhaustion Tier 2
                        skip_entry_timing=_skip_et,
                        # v42.1: ET Exhaustion Tier 1
                        et_exhaustion_tier1=_exhaustion_tier1,
                    )

                    # v42.0: Log Tier 2 exhaustion bypass after analyze() returns
                    if _skip_et and signal_data.get('signal') in ('LONG', 'SHORT'):
                        _prev_count = self._et_consecutive_rejects
                        self._et_consecutive_rejects = max(0, self._et_consecutive_rejects - 3)
                        self.log.warning(
                            f"⚡ v42.0: ET Exhaustion Tier 2 — signal {signal_data.get('signal')} "
                            f"passed through (ET skipped). Counter: {_prev_count} → {self._et_consecutive_rejects}"
                        )
                        signal_data['_et_exhaustion_tier2'] = True
                        signal_data['_et_exhaustion_rejects'] = _prev_count
                        # v42.0 fix: Clear dedup fingerprint so exhaustion-overridden signal
                        # bypasses dedup check. Without this, Tier 2 preserves Judge's original
                        # confidence (e.g., MEDIUM) which matches _last_executed_fingerprint,
                        # causing dedup to waste the exhaustion override.
                        self._last_executed_fingerprint = None
                        if self.telegram_bot and self.enable_telegram:
                            try:
                                _sig_cn = self.telegram_bot.side_to_cn(signal_data['signal'], 'side')
                                self.telegram_bot.send_message_sync(
                                    f"⚡ ET Exhaustion 放行 (Tier 2)\n"
                                    f"信号: {_sig_cn} | 连续拦截: {_prev_count} 次\n"
                                    f"ET 完全跳过，Judge confidence 保留",
                                    broadcast=False,
                                )
                            except Exception as e:
                                self.log.debug(f"ET exhaustion Tier 2 Telegram failed: {e}")
                    ...
                    if signal_data.get('debate_summary'):
                        self.log.info(f"🗣️ Debate Summary: {signal_data['debate_summary']}")
    ...
    def _save_decision_snapshot(
        ...
        try:
            ...
            # v27.0: Attach schema audit metadata for replay traceability
            _schema_audit = {}
            try:
                _schema_audit = self.multi_agent.get_schema_audit_metadata()
            except Exception as e:
                self.log.debug(f"Schema audit metadata not available: {e}")
            ...
            latest_signal = {
                ...
                'take_profit': signal_data.get('take_profit'),
                'debate_summary': signal_data.get('debate_summary', ''),
                'snapshot_id': _schema_audit.get('snapshot_id', ''),
                'schema_version': _schema_audit.get('schema_version', ''),
            }
            ...
            # v14.0: Phase timeline from multi-agent analyzer
            phase_timeline = {}
            if self.multi_agent:
                phase_timeline = getattr(self.multi_agent, 'last_phase_timeline', {})
    ...
    def _send_heartbeat_notification(self):
        ...
        try:
            ...
            heartbeat_msg = self.telegram_bot.format_heartbeat_message({
                ...
                # v23.0: Entry Timing Agent status
                'entry_timing': {
                    'verdict': getattr(self, '_entry_timing_verdict', ''),
                    'quality': getattr(self, '_entry_timing_quality', ''),
                } if getattr(self, '_entry_timing_verdict', '') else None,
                # v24.0: AI Quality Audit summary
                'ai_quality': self._get_ai_quality_summary(),
                # v35.0: Layer 3 outcome feedback summary
                'layer3': self._get_layer3_heartbeat_summary(),
    ...
    def _get_ai_quality_summary(self):
        """v24.0: Get AI Quality Audit summary for heartbeat display."""
        try:
            if hasattr(self, 'multi_agent') and self.multi_agent:
                report = getattr(self.multi_agent, 'last_quality_report', None)
                if report:
                    return {
                        'score': report.get('overall_score', 0),
                        'regime': report.get('regime', ''),
                        'flags_count': len(report.get('flags', [])),
                        # v26.0: error type breakdown
                        'citation_errors': report.get('citation_errors', 0),
                        'value_errors': report.get('value_errors', 0),
                        'zone_errors': report.get('zone_errors', 0),
                    }
        except Exception as e:
            self.log.debug(f"AI quality summary unavailable: {e}")
        return None

    def _get_layer3_heartbeat_summary(self):
        """v35.0: Get Layer 3 outcome feedback summary for heartbeat display."""
        try:
            from pathlib import Path
            from utils.quality_analysis import load_trades_from_file, get_heartbeat_summary
            data_dir = Path(__file__).parent.parent / 'data'
            trades = load_trades_from_file(data_dir)
            return get_heartbeat_summary(trades)
        except Exception as e:
            self.log.debug(f"Layer 3 summary unavailable: {e}")
        return None
        ...
        try:
            ...
            try:
                if hasattr(self, 'multi_agent') and self.multi_agent:
                    ...
                    self.log.info("📝 Ghost trade outcome recorded for AI learning")

                    # Queue reflection
                    if self.multi_agent.decision_memory:
                        last_mem = self.multi_agent.decision_memory[-1]
                        self._pending_reflections.append({
                            'timestamp': last_mem.get('timestamp', ''),
                            'memory_entry': last_mem,
                        })
            except Exception as e:
