...
class EventHandlersMixin:
    ...
    def on_position_opened(self, event):
        ...
        # v12.0: Capture Judge decision summary at entry for reflection context
        # v23.0: Also capture Entry Timing assessment
        if hasattr(self, 'latest_signal_data') and self.latest_signal_data:
            judge_dec = self.latest_signal_data.get('judge_decision', {})
            if judge_dec:
                rationale = judge_dec.get('rationale', '')
                risks = judge_dec.get('acknowledged_risks', [])
                ws = judge_dec.get('winning_side', '')
                self._entry_winning_side = ws
                risks_str = ', '.join(risks[:3]) if risks else ''
                self._entry_judge_summary = f"{rationale} | Risks: {risks_str}" if rationale else ""
            else:
                self._entry_winning_side = ""
                self._entry_judge_summary = ""
            # v23.0: Capture Entry Timing data for memory/reflection
            _ta = self.latest_signal_data.get('_timing_assessment', {})
            self._entry_timing_verdict = _ta.get('timing_verdict', '')
            self._entry_timing_quality = _ta.get('timing_quality', '')
            # v27.0: Capture structured debate tags for memory lesson tagging
            _sd = self.latest_signal_data.get('_structured_debate', {})
            if _sd:
                bull_tags = _sd.get('bull', {}).get('evidence', [])
                bear_tags = _sd.get('bear', {}).get('evidence', [])
                judge_tags = self.latest_signal_data.get('judge_decision', {}).get('decisive_reasons', [])
                self._entry_key_lesson_tags = list(set(bull_tags + bear_tags + judge_tags))[:10]
            else:
                self._entry_key_lesson_tags = []
            # v29+: Snapshot AnalysisContext data for memory
            ...
            self._entry_winning_side = ""
            self._entry_judge_summary = ""
            self._entry_timing_verdict = ""
            self._entry_timing_quality = ""
            self._entry_key_lesson_tags = []
    ...
    def on_position_closed(self, event):
        ...
        try:
            if hasattr(self, 'multi_agent') and self.multi_agent:
                ...
                try:
                    entry_ws = getattr(self, '_entry_winning_side', '')
                    entry_js = getattr(self, '_entry_judge_summary', '')
                    entry_tv = getattr(self, '_entry_timing_verdict', '')
                    entry_tq = getattr(self, '_entry_timing_quality', '')
                    self.multi_agent.record_outcome(
                        ...
                        winning_side=entry_ws,
                        entry_judge_summary=entry_js,
                        entry_timing_verdict=entry_tv,
                        entry_timing_quality=entry_tq,
                        close_reason=close_reason,
                        key_lesson_tags=getattr(self, '_entry_key_lesson_tags', None),  # v27.0
                        conditions_v2=getattr(self, '_entry_memory_conditions', None),  # v29+
                        ai_quality_score=getattr(self, '_entry_ai_quality_score', None),  # v29+
                    )
                    self.log.info(f"📝 Trade outcome recorded for AI learning")

                    # v12.0: Queue pending reflection for next on_timer cycle
                    # Uses the timestamp from the memory entry just created
                    if self.multi_agent.decision_memory:
                        ...
                        self.log.info(
                            f"🔍 Reflection queued (ts={last_mem.get('timestamp', '')[:19]})"
                        )
