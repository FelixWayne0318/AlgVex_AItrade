...
class OrderExecutionMixin:
    ...
    def _execute_trade(
        ...
        # v21.0: FR consecutive block exhaustion — degrade same-direction signal to HOLD
        # When FR has blocked the same direction >= 3 consecutive times, the market is
        # structurally hostile to that direction. Continuing to signal the same way
        # wastes AI calls and creates dead loops (e.g., Feb 28 scenario: 12× SHORT blocked).
        if (self._fr_consecutive_blocks >= 3
                and signal in ('LONG', 'SHORT')
                and not current_position):
            blocked_signal = 'LONG' if self._fr_block_direction == 'long' else 'SHORT'
            if signal == blocked_signal:
                self.log.warning(
                    f"🔄 FR exhaustion: {signal} degraded to HOLD — "
                    f"FR has blocked {self._fr_block_direction.upper()} "
                    f"{self._fr_consecutive_blocks}× consecutively. "
                    f"Market structurally hostile to this direction."
                )
                _block_dir_cn = self.telegram_bot.side_to_cn(self._fr_block_direction.upper(), 'side') if self._fr_block_direction.upper() in ('LONG', 'SHORT') else self._fr_block_direction.upper()
                _sig_cn = self.telegram_bot.side_to_cn(signal, 'side') if signal in ('LONG', 'SHORT') else signal
                self._last_signal_status = {
                    'executed': False,
                    'reason': (
                        f'FR 趋势疲劳 (连续 {self._fr_consecutive_blocks} 次 '
                        f'{_block_dir_cn} 被 FR 阻止)'
                    ),
                    'action_taken': '',
                }
                # Notify via Telegram
                if self.telegram_bot and self.enable_telegram:
                    try:
                        self.telegram_bot.send_message_sync(
                            f"🔄 FR 趋势疲劳检测\n\n"
                            f"AI 信号: {_sig_cn} → 降级为 HOLD\n"
                            f"原因: FR 连续阻止 {_block_dir_cn} "
                            f"方向 {self._fr_consecutive_blocks} 次\n"
                            f"建议: 等待 FR 方向反转或市场结构变化",
                            broadcast=False,
                        )
                    except Exception as e:
                        self.log.debug(f"FR exhaustion Telegram notification failed: {e}")
                return
        ...
        if self.telegram_bot and self.enable_telegram:
            ...
            self._pending_execution_data = {
                ...
                # v29.0: AI quality score for Telegram display
                'quality_score': signal_data.get('_quality_score'),
            }
        ...
        if target_side == current_side:
            ...
            if size_diff > 0:
                ...
                if current_price > 0 and layer_ratio > 0:
                    ...
                    layer_btc = round(layer_usdt / current_price, 3)
                ...
                if not sl_order_id:
                    ...
                    if self._layer_orders:
                        emg_layer_id = max(self._layer_orders.keys())
    ...
    def _validate_sltp_for_entry(
        ...
        # v11.0: Mechanical SL/TP calculation
        # v39.0: Pass 4H ATR as primary source (matches decision timeframe)
    ...
    def _open_new_position(self, side: str, quantity: float):
        ...
        if paying_funding and fr_abs > max_fr * 3:
            ...
            self.log.warning(
                f"🚫 Entry blocked: Funding Rate {fr_pct:.5f}% too high for {side.upper()} "
                f"(paying side, |FR|={fr_abs:.5f} > {max_fr * 3:.5f})"
            )
            ...
            self._last_signal_status = {
                'executed': False,
                'reason': f'Funding Rate 过高 ({fr_pct:.5f}%) 不利于 {side.upper()} (连续第{self._fr_consecutive_blocks}次)',
                'action_taken': '',
            }
            return
