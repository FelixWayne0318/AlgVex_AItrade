...
class PositionManagerMixin:
    ...
    def _reconcile_layer_quantities(self, current_position: Dict[str, Any]):
        ...
        for layer_id, layer in self._layer_orders.items():
            ...
            new_qty = round(old_qty * scale_factor, 4)
            if new_qty != old_qty:
    ...
    def _process_pending_reflections(self):
        """
        v12.0: Process queued reflections using LLM before AI analysis.

        Called at the start of on_timer. For each pending reflection:
        1. Calls multi_agent.generate_reflection() to get LLM-based deep reflection
        2. Calls multi_agent.update_last_memory_reflection() to replace template lesson
        3. Clears the pending queue

        v12.0.1: On first call after restart, scans decision_memory for recent entries
        missing 'reflection' field and backfills them (max 3 to limit API calls).

        If LLM call fails, template lesson remains (graceful degradation).
        """
        # Reflection config (v12.0)
        reflection_max_chars = 150
        reflection_temperature = 0.3

        if not hasattr(self, 'multi_agent') or not self.multi_agent:
            self._pending_reflections.clear()
            return

        # v12.0.1: Backfill missing reflections after restart (one-time scan)
        if not getattr(self, '_reflection_backfill_done', False):
            self._reflection_backfill_done = True
            try:
                # Scan last 10 entries for missing reflections (max backfill 3)
                backfill_count = 0
                for mem in reversed(self.multi_agent.decision_memory[-10:]):
                    if backfill_count >= 3:
                        break
                    if mem.get('evaluation') and not mem.get('reflection'):
                        ...
                        if ts:
                            ...
                            backfill_count += 1
                if backfill_count > 0:
                    self.log.info(f"🔍 Backfilling {backfill_count} missing reflection(s) from restart")
            except Exception as e:
                self.log.warning(f"🔍 Reflection backfill scan failed: {e}")

        if not self._pending_reflections:
            return

        count = len(self._pending_reflections)
        self.log.info(f"🔍 Processing {count} pending reflection(s)...")

        for pending in self._pending_reflections:
            ts = pending.get('timestamp', '')
            mem_entry = pending.get('memory_entry', {})
            if not ts or not mem_entry:
                continue

            try:
                reflection = self.multi_agent.generate_reflection(
                    memory_entry=mem_entry,
                    max_chars=reflection_max_chars,
                    temperature=reflection_temperature,
                )
                if reflection:
                    updated = self.multi_agent.update_last_memory_reflection(
                        target_timestamp=ts,
                        reflection=reflection,
                    )
                    if updated:
                        self.log.info(f"🔍 Reflection applied (ts={ts[:19]})")
                    else:
                        self.log.warning(f"🔍 Reflection generated but memory not found (ts={ts[:19]})")
                else:
                    self.log.debug(f"🔍 No reflection generated (ts={ts[:19]}) — keeping template")
            except Exception as e:
                self.log.warning(f"🔍 Reflection failed (ts={ts[:19]}): {e} — keeping template")

        self._pending_reflections.clear()

        # v18.0: After individual reflections, check extended reflection
        if hasattr(self, 'multi_agent') and self.multi_agent:
            ext_entry = self.multi_agent.check_and_generate_extended_reflection()
            if ext_entry and self.telegram_bot:
                insight = ext_entry.get('insight', '')
                try:
                    self.telegram_bot.send_message(
                        f"🔄 Extended Reflection 已生成\n"
                        f"覆盖最近 {ext_entry.get('trade_count', '?')} 笔交易 "
                        f"(胜率 {ext_entry.get('win_rate', 0)*100:.0f}%, "
                        f"avg R/R {ext_entry.get('avg_rr', 0):.1f}:1)\n"
                        f"---\n{insight}",
                        broadcast=False,  # Private chat only, for ops monitoring
                    )
                except Exception as e:
                    self.log.debug(f"Extended reflection notification failed (non-critical): {e}")
