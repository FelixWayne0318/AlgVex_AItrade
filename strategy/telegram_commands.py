...
class TelegramCommandsMixin:
    ...
    def _cmd_risk(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # v23.0/v42.0: Entry Timing consecutive REJECT counter + exhaustion status
            if self._et_consecutive_rejects > 0:
                msg += f"• ET 拦截: ⚠️ 连续 {self._et_consecutive_rejects}×"
                if self._et_consecutive_rejects >= self._ET_EXHAUSTION_TIER2:
                    msg += f" (Tier 2: 下次跳过 ET)"
                elif self._et_consecutive_rejects >= self._ET_EXHAUSTION_TIER1:
                    msg += f" (Tier 1: 下次覆盖 REJECT)"
                elif self._et_consecutive_rejects >= 4:
                    msg += " (可能死循环)"
                msg += "\n"
    ...
    def _cmd_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _thinking = getattr(self.config, 'deepseek_thinking_enabled', False)
            msg += f"  Thinking: {'✅' if _thinking else '❌'}\n"
    ...
    def _cmd_layer3(self) -> Dict[str, Any]:
        """Handle /layer3 command - show Layer 3 outcome feedback analysis."""
        try:
            ...
            if not trades:
                return {'success': True, 'message': '📊 *Layer 3 结果反馈*\n\n⚠️ 暂无交易数据'}
            ...
            for level in ['HIGH', 'MEDIUM', 'LOW']:
                ...
                if d.get('n', 0) == 0:
                    continue
                cal_wr = f"{d['win_rate']:.1%}" if d.get('win_rate') is not None else "N/A"
                ev_data = recal.get(level, {})
                ev = ev_data.get('expected_ev')
                ev_str = f", EV={ev:+.4f}" if ev is not None else ""
                msg += f"  {level}: {d.get('wins', 0)}/{d['n']} ({cal_wr}){ev_str}\n"
            flags = recal.get('flags', [])
            for flag in flags:
                msg += f"  ⚠️ {flag}\n"
            if not flags and any(conf_cal.get(l, {}).get('n', 0) > 0 for l in ['HIGH', 'MEDIUM', 'LOW']):
                msg += "  ✅ 信心校准正常\n"

            # Entry Timing
            et = analyze_entry_timing(trades)
            enter_d = et.get('ENTER', {})
            reject_d = et.get('REJECT', {})
            if enter_d.get('n', 0) > 0 or reject_d.get('n', 0) > 0:
                msg += "\n*入场时机效果*\n"
                if enter_d.get('n', 0) > 0:
                    e_wr = f"{enter_d['win_rate']:.1%}" if enter_d.get('win_rate') is not None else "N/A"
                    msg += f"  通过: {enter_d.get('wins', 0)}/{enter_d['n']} ({e_wr})\n"
                if reject_d.get('n', 0) > 0:
                    r_wr = f"{reject_d['win_rate']:.1%}" if reject_d.get('win_rate') is not None else "N/A"
                    msg += f"  拦截: {reject_d.get('wins', 0)}/{reject_d['n']} ({r_wr})\n"

            # Counter-trend
            ct = analyze_counter_trend(trades)
            tf = ct.get('trend_following', {})
            ctd = ct.get('counter_trend', {})
            if tf.get('n', 0) > 0 or ctd.get('n', 0) > 0:
                msg += "\n*顺势 vs 逆势*\n"
                if tf.get('n', 0) > 0:
                    t_wr = f"{tf['win_rate']:.1%}" if tf.get('win_rate') is not None else "N/A"
                    msg += f"  顺势: {tf.get('wins', 0)}/{tf['n']} ({t_wr}), R/R={tf.get('avg_rr', 0):.2f}\n"
                if ctd.get('n', 0) > 0:
                    c_wr = f"{ctd['win_rate']:.1%}" if ctd.get('win_rate') is not None else "N/A"
                    msg += f"  逆势: {ctd.get('wins', 0)}/{ctd['n']} ({c_wr}), R/R={ctd.get('avg_rr', 0):.2f}\n"

            # Grade distribution
            grades = analyze_grade_distribution(trades)
            if grades:
                msg += "\n*评级分布*\n"
                grade_parts = [f"{g}:{c}" for g, c in grades.items()]
                msg += f"  {' | '.join(grade_parts)}\n"

            # HOLD counterfactuals
            counterfactuals = load_counterfactuals_from_file(data_dir)
            if counterfactuals:
                hc = analyze_hold_counterfactuals(counterfactuals)
                if hc.get('total', 0) > 0:
                    by_verdict = hc.get('by_verdict', {})
                    correct = by_verdict.get('correct', 0)
                    wrong = by_verdict.get('wrong', 0)
                    decided = correct + wrong
                    acc = f"{correct / decided:.1%}" if decided > 0 else "N/A"
                    msg += f"\n*HOLD 反事实* ({hc['total']}次)\n"
                    msg += f"  正确: {correct} | 错误: {wrong} | 准确率: {acc}\n"

            return {'success': True, 'message': msg}
        except Exception as e:
            self.log.warning(f"Layer3 command failed: {e}")
            return {'success': False, 'error': str(e)}
