...
class TestAlignmentCap:
    """Phase 5a/5c: Alignment enforcement with TRANSITIONING."""

    @pytest.mark.skipif(not _openai_available, reason="openai not installed")
    def test_transitioning_aligned_1_allows_trade(self):
        """TRANSITIONING + aligned=1 → allows LOW confidence trade."""
        ...
        decision = {"decision": "LONG", "confidence": "MEDIUM"}
        confluence = {"aligned_layers": 1}
        dim_scores = {"regime_transition": "TRANSITIONING_BULLISH"}

        capped = analyzer._enforce_alignment_cap(decision, confluence, dim_scores)
        assert capped is True
        assert decision["confidence"] == "LOW"
        assert decision["decision"] == "LONG"  # Trade allowed

    @pytest.mark.skipif(not _openai_available, reason="openai not installed")
    def test_transitioning_aligned_0_forced_hold(self):
        """TRANSITIONING + aligned=0 → forced HOLD (Phase 5c)."""
        from agents.multi_agent_analyzer import MultiAgentAnalyzer
        import logging
    ...
    @pytest.mark.skipif(not _openai_available, reason="openai not installed")
    def test_no_transition_standard_cap(self):
        """No TRANSITIONING + aligned=1 → standard LOW cap (not HOLD)."""
        from agents.multi_agent_analyzer import MultiAgentAnalyzer
        import logging
        ...
        decision = {"decision": "LONG", "confidence": "HIGH"}
        confluence = {"aligned_layers": 1}
        dim_scores = {"regime_transition": "NONE"}

        capped = analyzer._enforce_alignment_cap(decision, confluence, dim_scores)
        assert capped is True
        assert decision["confidence"] == "LOW"
        assert decision["decision"] == "LONG"  # Still LONG, just LOW confidence
