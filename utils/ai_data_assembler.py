...
class AIDataAssembler:
    ...
    def fetch_external_data(
        ...
        # fetch_all() only returned current OI + liquidations, leaving _format_derivatives_report()'s
        # OI history formatting (lines 5031-5042) and L/S ratio history (lines 5157-5168) as dead code.
    ...
    def format_complete_report(self, data: Dict[str, Any]) -> str:
        """
        DEPRECATED: 生产代码从未使用此方法。AI prompt 在 multi_agent_analyzer.py 内
        通过 _format_technical_report()/_format_derivatives_report() 等方法构建。
