...
class SRZoneCalculator:
        ...
        for i in range(left, n - right):
            ...
            if is_swing_high:
                ...
                if bar_high >= current_price:
                    ...
                    _is_flip_h = True
                # Phase 1.7: Flip zones — look up persistent discount from cache.
                # Starts at 0.5; recovers +0.25 per confirmed new-direction touch (max 1.0).
                if _is_flip_h:
                    _cache_key_h = (round(bar_high, 0), side)
                    _fw_h = self._flip_discount_cache.get(_cache_key_h, 0.5)
                    self._current_flip_candidates.append((_cache_key_h, bar_high))
                else:
                    _fw_h = 1.0
                candidates.append(SRCandidate(
                    price=bar_high,
                    source=f"Swing_High",
                    weight=self.WEIGHTS['Swing_High'] * age_factor * _fw_h,
                    side=side,
                    extra={
                        'bar_index': i, 'bars_ago': bars_ago, 'age_factor': age_factor,
                        'is_flipped': _is_flip_h,
                        'flip_discount': _fw_h if _is_flip_h else 1.0,
                    },
                    level=SRLevel.INTERMEDIATE,
                    source_type=SRSourceType.STRUCTURAL,
                    timeframe="30m",  # v4.0 (B3), v18.2: 15M→30M
                ))
            ...
            if is_swing_low:
                ...
                if bar_low <= current_price:
                    ...
                    _is_flip_l = True
                # Phase 1.7: look up persistent discount
                if _is_flip_l:
                    _cache_key_l = (round(bar_low, 0), side)
                    _fw_l = self._flip_discount_cache.get(_cache_key_l, 0.5)
                    self._current_flip_candidates.append((_cache_key_l, bar_low))
                else:
                    _fw_l = 1.0
                candidates.append(SRCandidate(
                    price=bar_low,
                    source=f"Swing_Low",
                    weight=self.WEIGHTS['Swing_Low'] * age_factor * _fw_l,
                    side=side,
                    extra={
                        'bar_index': i, 'bars_ago': bars_ago, 'age_factor': age_factor,
                        'is_flipped': _is_flip_l,
                        'flip_discount': _fw_l if _is_flip_l else 1.0,
                    },
                    level=SRLevel.INTERMEDIATE,
                    source_type=SRSourceType.STRUCTURAL,
                    timeframe="30m",  # v4.0 (B3), v18.2: 15M→30M
    ...
    def generate_ai_detailed_report(
        ...
        if sma_data:
            ...
            if sma_50:
                lines.append(f"  • SMA_50 (30M-based, ~25h):  ${sma_50:,.2f}")
            if sma_200:
                lines.append(f"  • SMA_200 (30M-based, ~100h):  ${sma_200:,.2f}  ⚠️ NOT daily SMA_200")
        else:
        ...
        # =================================================================
        # Section 2: KEY S/R LEVELS (v17.0: 1 support + 1 resistance)
        # =================================================================
        lines.append("【KEY S/R LEVELS】")
        lines.append("-" * 40)

        def _format_zone_detail(zone, label):
            """Format a single zone with full detail."""
            zone_lines = []
            ...
            if zone.source_type == SRSourceType.PROJECTED:
                ...
                proj_tag = " ✅ CONFIRMED"
            # v8.0: Hold probability
            hold_tag = f" | Hold: {zone.hold_probability:.0%}" if zone.hold_probability > 0 else ""
            zone_lines.append(f"[{label}] ${zone.price_center:,.0f} ({zone.distance_pct:.1f}% from current) [{zone.level}|{zone.strength}]{proj_tag}")
            zone_lines.append(f"      Weight: {zone.total_weight} | Sources: {', '.join(zone.sources)}{hold_tag}")
            if zone.source_type == SRSourceType.PROJECTED:
                zone_lines.append(f"      ⚠️ Mathematical projection, NO historical trade confirmation")
            if zone.has_swing_point:
                zone_lines.append(f"      Swing Point: YES (structurally validated)")
            if zone.touch_count > 0:
                ...
                zone_lines.append(f"      Touch Count: {zone.touch_count} ({touch_quality})")
            if zone.has_order_wall:
                zone_lines.append(f"      Order Wall: {zone.wall_size_btc:.2f} BTC total")
                for wall in zone.order_walls:
                    zone_lines.append(f"        - ${wall['price']:,.0f}: {wall['size_btc']:.2f} BTC")
            zone_lines.append("")
            return zone_lines

        if nearest_resistance:
            lines.extend(_format_zone_detail(nearest_resistance, "R1"))
        else:
            lines.append("RESISTANCE: None detected")
            lines.append("")

        if nearest_support:
            lines.extend(_format_zone_detail(nearest_support, "S1"))
        else:
        ...
        if nearest_resistance:
            lines.append(f"Nearest Resistance: ${nearest_resistance.price_center:,.0f} "
                        f"({nearest_resistance.distance_pct:.1f}% away)")
            lines.append(f"  → {nearest_resistance.strength} resistance zone - potential rejection/reversal area")
            lines.append(f"  → Price approaching this level may face selling pressure")
        ...
        if nearest_support:
            lines.append(f"Nearest Support: ${nearest_support.price_center:,.0f} "
                        f"({nearest_support.distance_pct:.1f}% away)")
            lines.append(f"  → {nearest_support.strength} support zone - potential bounce/reversal area")
            lines.append(f"  → Price approaching this level may find buying pressure")
        ...
        if nearest_support and nearest_resistance:
            ...
            # Risk/reward ratio with guidance (cap extreme values when price hugs a zone)
            if current_to_support > 0:
                rr_ratio = current_to_resistance / current_to_support
                if rr_ratio > 20:
                    lines.append(f"  • LONG R/R Ratio: >20:1 ⚠️ UNRELIABLE (price too close to support)")
                else:
            ...
            if current_to_resistance > 0:
                ...
                if rr_ratio_short > 20:
                    lines.append(f"  • SHORT R/R Ratio: >20:1 ⚠️ UNRELIABLE (price too close to resistance)")
                else:
        ...
        lines.append("")
        lines.append("=" * 70)
        lines.append("NOTE: Only the nearest qualified support and resistance are shown.")
        lines.append("✅ CONFIRMED = STRUCTURAL (Swing Points) or ORDER_FLOW (real-time order book).")
