...
class TelegramBot:
            ...
            if executed and action_taken:
                ...
                if _hold_src:
                    _src_cn = {
                        ...
                        'risk_breaker': '风控熔断',
                        'et_reject': '入场时机拦截',
                        'explicit_judge': 'AI 判断',
                        'confidence_gate': '信心不足',
                    }.get(_hold_src, _hold_src)
                    msg += f" [{_src_cn}]"

        # ======= ENTRY TIMING STATUS (v23.0) =======
        et_data = heartbeat_data.get('entry_timing')
        if et_data and has_position:
            et_verdict = et_data.get('verdict', '')
            et_quality = et_data.get('quality', '')
            if et_verdict:
                et_icon = '✅' if et_verdict == 'ENTER' else '❌'
                et_cn = '通过' if et_verdict == 'ENTER' else '拦截'
                tq_cn = {'OPTIMAL': '最佳', 'GOOD': '良好', 'FAIR': '一般', 'POOR': '较差'}.get(et_quality, et_quality)
                msg += f"\n⏱ 入场时机: {et_icon} {et_cn} ({tq_cn})"

        # ======= AI QUALITY AUDIT (v24.0 + v26.0 error breakdown) =======
        ai_quality = heartbeat_data.get('ai_quality')
        if ai_quality:
            q_score = ai_quality.get('score', 0)
            q_icon = '🟢' if q_score >= 90 else '🟡' if q_score >= 70 else '🔴'
            q_line = f"\n{q_icon} AI质量: {q_score}/100"
            # v26.0: show error type breakdown instead of just flags count
            _cite = ai_quality.get('citation_errors', 0)
            _val = ai_quality.get('value_errors', 0)
            _zone = ai_quality.get('zone_errors', 0)
            _err_parts = []
            if _cite > 0:
                _err_parts.append(f"比较反转×{_cite}")
            if _val > 0:
                _err_parts.append(f"数值偏差×{_val}")
            if _zone > 0:
                _err_parts.append(f"区域错误×{_zone}")
            if _err_parts:
                q_line += f" ⚠️{'|'.join(_err_parts)}"
            msg += q_line
    ...
    def format_trade_execution(self, execution_data: Dict[str, Any]) -> str:
        ...
        if confluence and confluence.get('aligned_layers') is not None:
            ...
            layers = []
            for key, label in [('trend_1d', '1D'), ('momentum_4h', '4H'), ('levels_30m', '30M'), ('derivatives', '衍')]:
        ...
        # v23.0: Entry Timing Agent assessment
        timing_verdict = execution_data.get('timing_verdict', '')
        timing_quality = execution_data.get('timing_quality', '')
        if timing_verdict:
            tv_icon = '✅' if timing_verdict == 'ENTER' else '❌'
            tv_cn = '通过' if timing_verdict == 'ENTER' else '拦截'
            tq_cn = {'OPTIMAL': '最佳', 'GOOD': '良好', 'FAIR': '一般', 'POOR': '较差'}.get(timing_quality, timing_quality)
            msg += f"⏱ 入场时机: {tv_icon} {tv_cn} ({tq_cn})\n"

        # v29.0: AI quality score + S/R calibration
        _meta_parts = []
        quality_score = execution_data.get('quality_score')
        if quality_score is not None:
            q_icon = '🟢' if quality_score >= 80 else '🟡' if quality_score >= 60 else '🔴'
            _meta_parts.append(f"{q_icon} AI质量 {quality_score}/100")
        ...
        if cal_data:
            ...
            if cal_stale:
                ...
                _meta_parts.append(f"📊 S/R校准 {cal_ver}")
        if _meta_parts:
            msg += f"\n{' | '.join(_meta_parts)}\n"

        # v29.0: Combined AI reasoning — Judge rationale + Risk reason
        # Judge rationale is the strategic analysis, Risk reason is the risk assessment
        judge_rationale = execution_data.get('judge_rationale', '')
        combined = ''
        if judge_rationale and reasoning:
            combined = f"📋 {judge_rationale}\n⚠️ {reasoning}"
        elif judge_rationale:
            combined = judge_rationale
        elif reasoning:
            combined = reasoning

        if combined:
            safe = self.escape_markdown(combined)
            msg += f"\n🤖 {safe}\n"
    ...
    def format_daily_summary(self, summary_data: Dict[str, Any]) -> str:
        ...
        # v12.0: Recent AI reflections summary
        recent_reflections = summary_data.get('recent_reflections', [])
        if recent_reflections:
            msg += f"\n🔍 *AI 反思*\n"
            for refl in recent_reflections[:3]:  # Show up to 3 most recent
                grade = refl.get('grade', '?')
                pnl = refl.get('pnl', 0)
                text = refl.get('reflection', '')
                # v30.6: Escape markdown in reflection text to prevent parse errors
                safe_text = self.escape_markdown(text)
                pnl_icon = '🟢' if pnl >= 0 else '🔴'
                msg += f"  {pnl_icon} [{grade}] {safe_text}\n"
    ...
    def format_weekly_summary(self, summary_data: Dict[str, Any]) -> str:
        ...
        if recent_reflections:
            msg += f"\n🔍 *AI 反思*\n"
            for refl in recent_reflections[:3]:
                grade = refl.get('grade', '?')
                pnl = refl.get('pnl', 0)
                text = refl.get('reflection', '')
                # v30.6: Escape markdown in reflection text to prevent parse errors
    ...
    def format_scaling_notification(self, scaling_info: Dict[str, Any]) -> str:
        ...
        # AI reasoning — full content, _split_message() handles Telegram 4096 limit
