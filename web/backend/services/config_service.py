...
class ConfigService:
    ...
    def get_config_sections(self) -> Dict[str, Any]:
        ...
        field_definitions = {
            ...
            "ai": [
                ...
                {"path": "ai.deepseek.thinking_enabled", "label": "Thinking Mode", "type": "boolean", "description": "Enable DeepSeek thinking mode for deeper reasoning (v32.0)"},
    ...
    def get_system_info(self) -> Dict:
        ...
        system_version = "v32.3"  # Current system version per CLAUDE.md

        # Read thinking mode status from config
        config = self.read_base_config()
        thinking_enabled = False
        try:
            thinking_enabled = config.get("ai", {}).get("deepseek", {}).get("enable_thinking", False)
