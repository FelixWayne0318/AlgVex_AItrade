...
import {
  Activity, Brain, Shield, Clock, TrendingUp, Layers,
...
const AIDecisionDetail = dynamic(
  () => import("@/components/admin/ai-decision-detail").then((mod) => mod.AIDecisionDetail),
...
export default function DashboardPage() {
  ...
  // AI signal history
  const { data: signalHistory } = useSWR(
    "/api/public/signal-history?limit=5",
  ...
  // Build signal log data
  const signals = toArray(signalHistory, 'signals').map(
    (s: any, i: number) => ({
      id: s.id || `signal-${i}`,
      timestamp: s.timestamp || new Date().toISOString(),
      symbol: s.symbol || "BTCUSDT",
      bull_analysis: typeof s.bull_analysis === 'object' && s.bull_analysis
        ? s.bull_analysis
        : {
            signal: s.signal || "HOLD",
            confidence: s.confidence || "MEDIUM",
            reasoning: s.bull_analysis || "No data",
          },
      bear_analysis: typeof s.bear_analysis === 'object' && s.bear_analysis
        ? s.bear_analysis
        : {
            signal: s.signal === "BUY" ? "SELL" : "BUY",
            confidence: s.confidence || "MEDIUM",
            reasoning: s.bear_analysis || "No data",
          },
      judge_decision: typeof s.judge_decision === 'object' && s.judge_decision
        ? s.judge_decision
        : {
            signal: s.signal || "HOLD",
            confidence: s.confidence || "MEDIUM",
            reasoning: s.judge_reasoning || s.reasoning || "No data",
            bull_score: s.bull_score,
            bear_score: s.bear_score,
          },
      final_signal: s.signal || s.final_signal || "HOLD",
      confidence: s.confidence || "MEDIUM",
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Real-time trading bot monitoring dashboard"
        />
      </Head>
      ...
      <div className="min-h-screen gradient-bg noise-overlay">
        ...
        <main className="pt-24 sm:pt-28 pb-12 px-4">
          <div className="container mx-auto max-w-7xl space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {locale === "zh"
                    ? "机器人状态、市场数据、AI 信号一览"
                    : "Bot status, market data, and AI signals at a glance"}
                </p>
              </div>
            </div>
            ...
            {/* Row 4: Layer Orders + AI Decision Detail */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card className="border-border/50">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
              ...
              <Card className="border-border/50">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                    ...
                    {locale === "zh" ? "AI 决策详情" : "AI Decision Detail"}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ErrorBoundary>
                    <AIDecisionDetail analysis={aiAnalysis} />
                  </ErrorBoundary>
                </CardContent>
              </Card>
            </div>
            ...
            {/* Row 6: AI Signals + Trade Timeline */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card className="border-border/50">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Brain className="h-4 w-4 text-primary" />
                    {locale === "zh" ? "AI 信号日志" : "AI Signal Log"}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ErrorBoundary>
                    {signals.length > 0 ? (
                      <AISignalLog signals={signals} maxItems={5} />
                    ) : (
